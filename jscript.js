$(document).ready(function() {
    ...
    {let i = 0; while (i < 10) {
    if ((parseInt($('#lane1').css('top')) < -10) || (parseInt($('#lane1').css('top')) > 470)) {
    $('#car1').fadeOut();
    // $('#lane1').fadeOut();
    // $('#lane1').animate({
    //	left: '0px',
    ...
    $('#lane1').animate({
    left: '+=' + Math.floor(Math.random() * (250 - 1) + 1) + 'px',top: '+=' + Math.floor(Math.random() * (100 - 1) + 1)
    + 'px',
    ...
    // let data = prompt("Enter Content: ");data = 'dynamic';
    $("#i1").val(data);
    })
